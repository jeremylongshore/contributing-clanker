...
- ("bounties", "<contribution_id>"): Individual bounty tracking
...
class MemoryManager:
    ...
    async def get_bounty_state(self, contribution_id: str) -> Optional[Dict[str, Any]]:
        """Get current bounty workflow state.

        Args:
            contribution_id: Unique bounty identifier
        ...
        try:
            result = await self.store.aget(
                namespace=(*NAMESPACE_BOUNTIES, contribution_id),
                key="state",
            )
            return result.value if result else None
        except Exception as e:
            logger.error(f"Failed to get bounty state {contribution_id}: {e}")
            return None

    async def save_bounty_state(
        self, contribution_id: str, state: Dict[str, Any]
    ) -> None:
        ...
        logger.info(f"Saved bounty state: {contribution_id} (phase: {state.get('phase', '?')})")
