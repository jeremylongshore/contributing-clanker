...
# Endpoints
@app.post("/api/contribution/start", response_model=StartWorkflowResponse)
async def start_contribute_workflow(
    ...
    """
    session_id = f"bounty-{request.contribution_id}-{uuid.uuid4().hex[:8]}"
    ...
    config = {"configurable": {"thread_id": request.contribution_id}}
...
@app.get("/api/contribution/{contribution_id}/status", response_model=WorkflowStatus)
async def get_bounty_status(contribution_id: str):
    """Get current workflow state for a bounty.

    Returns the current node, phase, and full state.
    """
    config = {"configurable": {"thread_id": contribution_id}}
...
@app.post("/api/contribution/{contribution_id}/approve", response_model=ApproveResponse)
async def approve_execution(contribution_id: str, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        return ApproveResponse(status="executing", contribution_id=contribution_id)
    except Exception as e:
        raise HTTPException(status_code=400, detail=f"Approval failed: {e}")


@app.post("/api/contribution/{contribution_id}/reject", response_model=ApproveResponse)
async def reject_execution(contribution_id: str):
    ...
    try:
        ...
        return ApproveResponse(status="rejected", contribution_id=contribution_id)
    except Exception as e:
...
async def get_bounty_detail(contribution_id: str):
    ...
    try:
        state = await memory.get_bounty_state(contribution_id)
        if not state:
...
async def record_outcome(contribution_id: str, request: RecordOutcomeRequest):
    ...
    try:
        ...
        return {"status": "recorded", "contribution_id": contribution_id, "outcome": request.outcome}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def health_check():
    ...
    return {"status": "healthy", "service": "contribute-orchestrator"}
